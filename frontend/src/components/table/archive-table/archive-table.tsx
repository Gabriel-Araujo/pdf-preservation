...
const download_action = (uuid: string) => {
    downloadFile(uuid)
        .then((res) => res?.blob())
        .then((blob) => blob ? window.open(window.URL.createObjectURL(blob)) : null);
}
...
const columns = [
    ...
    {
        id: "actions",
        cell: ({ row }: {row: CoreRow<ArchiveFile>}) => {const archive = row.original
            return (
                <div style={{display: "inline-flex", columnGap: "1rem"}}>
                    <button className={styles.actionButton} onClick={() => download_action(archive.uuid)}>
                    ...
                    <button className={styles.actionButton} onClick={() => redirect(`/file/${archive.uuid}`)}>
